...
const baseLayout = {
    ...
    xaxis: { showgrid: true, gridcolor: '#0c1a2c', linecolor: '#0d1e30', tickfont: { family: 'JetBrains Mono', color: '#3d5070', size: 9 }, dtick: 3 },
...
function updateChartsView(zonesObj) {
    ...
    const hc = (data.density * 50); // rough estimation for headcount
    document.getElementById("cv-headcount").innerText = hc.toFixed(2);
    document.getElementById("cv-risk").innerText = (data.risk_probability / 100).toFixed(2);
    ...
    // Plotly Updates
    if (hist.l.length > 1) {
        renderIndividualChart('chart-density', hist.l, hist.d, dColor, 'p/m²', 2.5);
        renderIndividualChart('chart-velocity', hist.l, hist.v, '#818CF8', 'm/s', 0.5); // ref inverted conceptually 
        
        let hcHist = hist.d.map(d => d * 50);
        renderIndividualChart('chart-headcount', hist.l, hcHist, '#22D3EE', '', info.cap);
        
        let riskHist = hist.r.map(r => r / 100);
        renderIndividualChart('chart-risk', hist.l, riskHist, '#F59E0B', '', 0.7);

        // Advanced AI Plots
        renderIndividualChart('chart-time', hist.l, hist.tc, '#c084fc', 'min', null);
        renderIndividualChart('chart-prob', hist.l, riskHist, '#F59E0B', '%', null);
        
        // Multi-line comparisons
        renderMultiChart('chart-all-density', 'd');
        renderMultiChart('chart-all-velocity', 'v');
    }
}
...
function renderIndividualChart(id, x, y, color, label, refVal) {
    ...
    const trace2 = { x: x, y: y, type: 'scatter', mode: 'lines', line: {color: color, width: 2, shape: 'hv'}, hoverinfo: 'none' };
    ...
    Plotly.react(id, [trace1, trace2], layout, {displayModeBar: false});
}

function renderMultiChart(id, property) {
    const traces = [];
    Object.keys(ZONE_INFO).forEach(z => {
        const hist = zoneHistories[z];
        const c = ZONE_INFO[z].color;
        traces.push({ x: hist.l, y: hist[property], name: ZONE_INFO[z].name_short, type: 'scatter', mode: 'lines', line: {color: c, width: 2, shape: 'spline'} });
    });
    
    const layout = JSON.parse(JSON.stringify(baseLayout));
    layout.showlegend = true; 
    layout.legend = { orientation: "h", y: -0.2, x: 0.5, xanchor: 'center', font: {size: 10, color: '#607898'}, bgcolor: 'transparent' };
    
    Plotly.react(id, traces, layout, {displayModeBar: false});
}
